...
:data:`PRELUDE` is emitted verbatim at the top of generated modules so that a
file produced by ``--dump-python`` is a completely standalone script with no
dependency on PyAsm itself.  The module also executes the prelude so the same
helpers can be imported and unit tested.
"""
...
#: Names the prelude defines; generated programs must not shadow them.
...
exec(compile(PRELUDE, "<pyasm-runtime>", "exec"), globals())  # noqa: S102
