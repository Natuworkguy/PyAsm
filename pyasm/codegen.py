...
def _end_for_cleans_up(program: Program) -> bool:
    ...
    3.12 popped it there.  3.13 emits ``END_FOR`` followed by a ``POP_TOP``
    that does the popping instead, and 3.14 renamed that ``POP_TOP`` to
    ``POP_ITER``; in both, ``END_FOR`` is a no-op for us.
    """
    instructions = program.instructions
    for index, instruction in enumerate(instructions):
        if instruction.opname != "END_FOR":
            continue
        after = instructions[index + 1:index + 2]
...
class _Generator:
    ...
    def __init__(self, program: Program, options: CodegenOptions) -> None:
        ...
        self.end_for_pops = _end_for_cleans_up(program)
...
def _end_for(gen: _Generator, ins: Instruction) -> None:
    # 3.12 pops the exhausted iterator here; 3.13 and 3.14 leave that to the
    # POP_TOP (POP_ITER from 3.14) that follows.
...
def _docstring_text(text: str) -> str:
    ...
    Windows paths are full of backslashes, and ``C:\\Users`` inside a
    docstring is a truncated ``\\U`` escape, not a path.
    """
...
def _render(
    ...
    out = _Writer()
    name = _docstring_text(options.source_name)
    out.add(f'"""Generated by PyAsm from {name}.')
    out.add("")
